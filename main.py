...
class Config
    ...
    API_URL = httpapi.alquran.cloudv1juz30quran-simple
...
class LocalizationManager
    TRANSLATIONS = {
        fa {
            ...
            bookmark_empty ☆ هنوز هیچ آیه‌ای را نشان‌گذاری نکرده‌اید! ☆nحین تمرین با کلیک روی ستاره خالی، آیات سخت را اضافه کنید.,
        ...
        en {
            ...
            wrong_answers Correct with AssistanceErrors {},
            ...
            bookmark_title ✨ BookmarkedDifficult Verses Quiz (⭐) ✨,
            bookmark_empty ☆ No bookmarked verses yet! ☆nClick the star icon during practice to add difficult verses.,
...
class AudioManager
    ...
    def _download_and_play_thread(self, global_num, file_path)
        ...
        audio_url = fhttpscdn.alquran.cloudmediaaudioayahar.husary{global_num}
        try
            session = requests.Session()
            session.trust_env = False  
            ...
            print(fBackground audio download failed {e})
    ...
    def _execute_audio_play(self, path)
        try
            ...
            print(fAudio player failed to load {e})
...
class QuranPageQuizApp
    ...
    def create_widgets(self)
        ...
        self.lang_combobox = ttk.Combobox(self.top_frame, values=[Persian  فارسی, English], state=readonly, width=14)
    ...
    def on_language_change(self, event)
        ...
        lang_code = fa if selected == 0 else en
    ...
    def load_page_data(self, reverse_audio=False)
        ...
        self.page_verses = self.current_verses[start_idxend_idx]
        ...
        for i, verse in enumerate(self.page_verses)
            ...
            star_id = self.canvas.create_text(x1 + 45, y1 + 30, text=star_char, font=(Arial, 14), fill=#D4AF37, tags=(star_btn, fstar_{v_key}))

            text_id = self.canvas.create_text(x1 + 240 - 15, y1 + 30, text=f({verse['numberInSurah']}), font=(Tahoma, 10, bold), fill=#2C5E43, anchor=e)
            audio_btn = self.canvas.create_text(x1 + 20, y1 + 30, text=🔊, font=(Arial, 12), fill=#C5A880, tags=(slot_audio_btn, fslot_id_{verse['numberInSurah']}))
        ...
        for i, verse in enumerate(shuffled_verses)
            ...
            tag_name = fcard_{verse['numberInSurah']}

            rect = self.canvas.create_rectangle(x1, y1, x2, y2, fill=#EFEBE4, outline=#8D6E63, width=1, tags=(draggable, tag_name))
            audio_btn = self.canvas.create_text(x1 + 18, y1 + card_h2, text=🔊, font=(Arial, 12), fill=#2C5E43, tags=(draggable, tag_name, audio_btn))
            
            card_text = self.loc.get(audio_drag) if reverse_audio else verse['text']
            text = self.canvas.create_text(x1 + card_w - 10, y1 + card_h2, text=card_text, font=(Arial, 11), fill=#3E2723, anchor=e, justify=right, width=card_w-45, tags=(draggable, tag_name))
    ...
    def start_neighbors_quiz(self, specific_index=None, custom_verses=None)
        ...
        for i, idx in enumerate(neighbor_indices)
            ...
            if idx == anchor_idx
                ...
                self.canvas.create_text(x1 + slot_w - 15, y1 + slot_h2, text=f({verse['numberInSurah']}), font=(Tahoma, 10, bold), fill=#2E7D32, anchor=e)
                self.canvas.create_text(x1 + 20, y1 + slot_h2, text=🔊, font=(Arial, 12), fill=#2E7D32, tags=(anchor_audio_btn, fanchor_audio_{verse['numberInSurah']}))
                self.canvas.create_text(x1 + slot_w - 45, y1 + slot_h2, text=verse['text'], font=(Arial, 11, bold), fill=#1b5e20, anchor=e, justify=right, width=slot_w-75)
                ...
                v_key = f{verse['surahNumber']}_{verse['numberInSurah']}
                star_char = ⭐ if v_key in progress_data[bookmarks] else ☆
                self.canvas.create_text(x1 + 45, y1 + slot_h2, text=star_char, font=(Arial, 14), fill=#D4AF37, tags=(star_btn, fstar_{v_key}))

                self.canvas.create_text(x1 + slot_w - 15, y1 + slot_h2, text=f({verse['numberInSurah']}), font=(Tahoma, 10, bold), fill=#2C5E43, anchor=e)
                self.canvas.create_text(x1 + 20, y1 + slot_h2, text=🔊, font=(Arial, 12), fill=#C5A880, tags=(slot_audio_btn, fslot_id_{verse['numberInSurah']}))
        ...
        for i, verse in enumerate(pool_verses)
            ...
            rect = self.canvas.create_rectangle(x1, y1, x2, y2, fill=#EFEBE4, outline=#8D6E63, width=1, tags=(draggable, tag_name))
            audio_btn = self.canvas.create_text(x1 + 18, y1 + card_h2, text=🔊, font=(Arial, 12), fill=#2C5E43, tags=(draggable, tag_name, audio_btn))
            text = self.canvas.create_text(x1 + card_w - 10, y1 + card_h2, text=verse['text'], font=(Arial, 11), fill=#3E2723, anchor=e, justify=right, width=card_w-45, tags=(draggable, tag_name))
    ...
    def start_completion_quiz(self, specific_index=None, custom_verses=None)
        ...
        self.canvas.create_text(480, 125, text=self.loc.get(audio_hint), font=(Tahoma, 10, bold), fill=#2C5E43, tags=(anchor_audio_btn, fanchor_audio_{selected_verse['numberInSurah']}))
        ...
        for i in range(3)
            ...
            if i == hidden_idx
                ...
                self.canvas.create_text(x1 + block_w2, y1 + block_h2, text=self.loc.get(empty_slot), font=(Tahoma, 10, italic), fill=#C5A880)
                
                v_key = f{selected_verse['surahNumber']}_{selected_verse['numberInSurah']}
                star_char = ⭐ if v_key in progress_data[bookmarks] else ☆
                self.canvas.create_text(x1 + 25, y1 + block_h2, text=star_char, font=(Arial, 14), fill=#D4AF37, tags=(star_btn, fstar_{v_key}))
                ...
                self.canvas.create_text(x1 + block_w2, y1 + block_h2, text=part_text, font=(Arial, 12), fill=#3E2723, width=block_w-15, justify=center)
        ...
        for i, item in enumerate(pool_items)
            ...
            tag_name = fcard_{item['num']}

            rect = self.canvas.create_rectangle(x1, y1, x2, y2, fill=#EFEBE4, outline=#8D6E63, width=1, tags=(draggable, tag_name))
            text = self.canvas.create_text(x1 + card_w2, y1 + card_h2, text=item['text'], font=(Arial, 11), fill=#3E2723, width=card_w-20, justify=center, tags=(draggable, tag_name))
